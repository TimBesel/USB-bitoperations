    ...
    for(uint8_t bIdx = 0; bIdx < dataSize; bIdx++){
        ...
        if((buffer & 0x7F) == 0x7F){
            ...
            *dataPointer = (*dataPointer << 1) | (0x00);
            ...
            dataBuffer = (packet_typ_t *)realloc(dataBuffer, ((size_t)ceil((dataSize + (++insertedBits)) / (8 * sizeof(packet_typ_t))) + 1) * sizeof(packet_typ_t));
...
packet_typ_t *nrzi(packet_typ_t *data, uint8_t dataSize){
    ...
    for(uint8_t bIdx = 0; bIdx < dataSize%(8 * sizeof(packet_typ_t)); bIdx++){
        uint8_t bit = (*(data + (uint8_t)ceil(dataSize/(8 * sizeof(packet_typ_t)))) >> ((dataSize % (8 * sizeof(packet_typ_t))) - (bIdx+1) )) & 0x01;
